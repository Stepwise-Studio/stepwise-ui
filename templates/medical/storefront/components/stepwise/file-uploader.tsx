    ...
    <ul className="flex flex-col gap-1.5 w-full">
      <AnimatePresence initial={false}>
        {files.map(f => (
          ...
          >
            {/* TEMPLATE FIX (see THEMING.md): the registry copy draws this edge
                with a CSS `border`, which the squircle clip-path slices off.
                Routed through middleBorder instead. */}
            <Surface
              radius={10}
  ...
  if (variant === 'dropzone') {
    ...
    return (
      <div className={cn('flex w-full flex-col items-center gap-4', className)}>
        ...
        {/* TEMPLATE FIX (see THEMING.md): the registry copy draws this box with
            `rounded-[28px] border-2 border-dashed`, which is both a plain
            corner radius and a CSS border. Both are replaced by a squircle with
            a dashed `middleBorder` - lisse strokes the dash along the actual
            squircle path, so the corners stay smooth and nothing is clipped.
            Keyed on the drag state so the stroke colour redraws on change. */}
        <SmoothCorners
          ...
          middleBorder={{
            ...
            gap    : 6,
            color  : dragging
              ? 'var(--ui-border-focus, rgb(113 113 122))'
